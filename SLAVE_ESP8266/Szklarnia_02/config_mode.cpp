...
void handleRoot()
{
  if (server.args() > 0) // Jeżeli podano jakieś parametry to:
  {
    ...
    // Zapisz nową strukturę na system SPIFFS:
    // Zamontuj system plików SPIFFS:
    if (SPIFFS.begin() == true)
    {
      Serial.println("Udało się zamontować system plików! Przygotowanie do zapisu...");
    }
    else
    {
      Serial.println("Nie udało się zamontować systemu plików! Nie udało się przygotować do zapisu");
    }

    // Otwórz i zapisz zawartość struktury 'config' do pliku '/struct_config.bin' - otwórz w trybie tylko do zapisu:
    File f = SPIFFS.open("/struct_config.bin", "w");
    if (f == true)
    {
      Serial.println("Udało się otworzyć plik konfiguracyjny do zapisu");
    }
    else
    {
      Serial.println("Nie udało się otworzyć pliku konfiguracyjnego do zapisu");
    }
    Serial.print("Do pliku zapisano ");
    Serial.print(f.write((uint8_t*)&config, sizeof(config)));
    Serial.println(" bajtów danych");

    // Wymuś zmiany w systemie plików:
    f.flush();
